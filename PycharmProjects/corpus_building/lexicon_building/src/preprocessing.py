...
from yutils.str_utils import segment_str
...
def sentence_to_index(word2idx, sentences):
    ...
    sentences = segment_str(sentences)
    sentences_indexes = []
...
def processing():
    output_dir = "../../data/pickle_data"
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    # read raw text
    fns = "../../data/corpus/cover_implicit"
    sentences = model_utils.read_file2list(fns + ".sentence")
    labels =  model_utils.read_file2list(fns + ".label")
    ######测试阶段##############
    sentences=sentences
    labels = labels
    ##########################
    ...
    model_utils.shuffle(data, seed=123456)
    assert len(data[0]) == len(data[1])

    # split the dataset: train, valid,test 在资源构建任务中可以尝试过拟合
    ...
    emb_file = emb_dir+"vectors100 .txt"
    ...
    model_utils.dict2pickle(datasets, output_dir + "/features_glove.pkl")
    model_utils.dict2pickle(word2idx, output_dir + "/word2idx_glove.pkl")
    model_utils.dict2pickle(embeddings, output_dir + "/embeddings_glove.pkl")
    model_utils.dict2pickle(idx2word, output_dir + "/idx2word_glove.pkl")
